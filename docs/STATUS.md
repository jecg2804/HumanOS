...
| SEC-LEGACY | ~~Schema legacy `humanos.*` con policies permisivas + 36 filas~~ **DONE (W3 066):** `DROP SCHEMA humanos CASCADE` (5 tablas/49 filas + fn + policies) tras snapshot atómico a `backup.humanos_*_20260602` (R22). Data ya en `hr.*`; 0 deps; advisors limpios de lints humanos. Prohibición se mantiene (anti-recreación) | Codex | P2 | DONE | security | — |
...
| ENV-SROLE | **Verificado read-only (W3 4b):** keys presentes; el proyecto tiene legacy + nuevas API keys (legacy NO disabled). `.env.local` mezcla anon NUEVO (`sb_publishable_`, 46c) + service_role LEGACY JWT (220c) → el "Invalid API key" local apunta a un VALOR stale/equivocado del service_role, no a key-migration. **Acción de Jaime:** refrescar el service_role local desde el dashboard (sin rotar) + confirmar Vercel (logs del cron) | Codex | P2 | BLOCKED-on-Jaime | security | acción local de Jaime |
