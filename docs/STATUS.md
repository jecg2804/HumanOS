...
| CODE-CRON | Worker de cron ignora `{ error }` en updates (`sent`/`markPermanent`/`markRetryable`) → estado falso, duplicados o reintentos eternos. **Batch 2 (W3):** fix = claim atómico (`status='sending'` antes del envío + reaper), NO solo loguear (loguear deja el duplicado) | Codex | P2 | OPEN | code | pre-Group-3 |
