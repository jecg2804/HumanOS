...
| HOOK-MCP-GAP | `pre-tool-use.ps1` CHECK 1 (R1 prohibidos + R22 auth.users) sólo matchea los tool-names `plugin_supabase_supabase` / `supabase`, NO `claude_ai_Supabase` (el MCP en uso esta sesión) → enforcement físico R1/R22 BYPASSED para la herramienta real (migraciones 063-066 no fueron hook-checked). Fix: agregar `claude_ai_supabase` al regex de tool-name + `drop schema` a writeKeywords | mine | P1 | OPEN | security | inmediato (harness) |
