...
| **W4** | FE-3 a11y full + FE-4 íconos PWA | **DONE** — FE-4 PWA (`29bdb67`) + FE-3 guard/labels parte 1 (`48fde02`) + FE-3 parte 2 manual ARIA (esta sesión) |
...
- **FUNDACIÓN verificada contra realidad (2026-06-02):** el primer login real end-to-end reveló 3 capas que estaban "verdes en papel" pero rotas: (1) **Exposed Schemas** — solo `public` estaba expuesto → la app nunca funcionó contra esta BD (0 logins históricos); Jaime expuso los schemas HumanOS. (2) **GRANT** — `hr.people` era la única tabla `hr.*` sin grant a `authenticated` → `permission denied` ANTES de RLS → "no perfil vinculado"; fix `067` (GRANT SELECT) + `069` (least-privilege, REVOKE writes — todas van por SECURITY DEFINER/service_role). (3) usuario real de Jaime `jcucalon@iconsanet.com` (UNA identidad cross-app, `allowed_apps=['movimientOS','humanOS']`) linkeado a su registro `CUC166`. **Auditoría de fundación de 6 agentes** (harness/docs/BD/migraciones/build/git) = sólida; los 2 "P1" de los agentes (login roto / over-granted) eran FALSOS (ground-truth: `authenticated=r`). Único P1 real = HOOK-MCP-GAP settings.json (backlog). `072` hardening (revoke EXECUTE de la trigger-fn `create_default_user_settings` + DENY explícito en `requests.sequences`). Docs de helper-list/grant-layer/Exposed-Schemas actualizados (`schemas-permisos.md`/`integrations.md`/Constitution/session-start). **Lección:** el config/guardrail de runtime (Exposed Schemas, grants, matcher del harness) NO es visible por SQL/código → verificar contra el sistema CORRIENDO, no contra reportes ni docs.
...
| HOOK-MCP-GAP | ~~`pre-tool-use`/`post-tool-use` no matcheaban `claude_ai_Supabase`~~ **DONE (W3 4a + foundation-audit):** (a) wildcard `mcp__.*supabase.*__` DENTRO de ambos `.ps1` + CHECK 1a-bis (`DROP SCHEMA public/payroll/humanos`). **(b) FIX REAL que faltaba (hallado por la auditoría de fundación): el TRIGGER del harness (`.claude/settings.json` matcher) seguía enumerando solo `plugin_supabase_supabase`/`supabase` → el hook NUNCA se invocaba para `claude_ai_Supabase` (el MCP en uso) pese al regex correcto del .ps1. El smoke-test por stdin probaba el script AISLADO, no el trigger del harness.** Matcher wildcardeado en `settings.json` (`a0d7d7f`); toma efecto en próxima sesión (settings cacheado al arranque) | mine | P1 | DONE | security | — |
...
| FE-3-full | **DONE (W4):** Guard (`eslint-plugin-jsx-a11y` recommended a `error`, piso permanente) + 12 violaciones (parte 1, `48fde02`). **Parte 2 manual ARIA (esta sesión):** progressbar role + focus-trap/Escape/restore (WizardLayout); skip-to-main (AppShell layout); caption + scope=col + aria-pressed (EmployeeList); new-tab aria-labels (Step9); avatar fallback role=img + iniciales (perfil); focus-visible rings + aria-expanded/haspopup (NotificationBell); Escape + outside-click + region label (NotificationDropdown, NO role=menu); label↔control association (CatalogComboboxField, select nativo → asociación, no role fake). Gate verde | mine | P3 | DONE | FE | — |
...
| A11Y-AXE-CI | `jsx-a11y` NO atrapa "control sin nombre accesible" (placeholder-only / `<label>` hermano sin `htmlFor`) — solo axe lo ve en runtime. Barrido 2026-06-02 arregló ~22 controles (EmployeeForm/Step6/Step3/7/8/forgot/reset/NotificationItem) pero la clase puede recurrir. Durable: agregar axe (`vitest-axe` o `@axe-core/playwright`) al gate/CI | mine | P3 | OPEN | FE/test | pasada A11Y |
