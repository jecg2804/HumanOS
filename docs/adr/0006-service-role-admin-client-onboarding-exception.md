...
> **Refinado (2026-06-03, audit F-12) — no se reescribe la historia, se anota el estado actual:** la decisión central (cliente service-role acotado a una server action nombrada, como excepción a ADR-0001) SIGUE vigente. Lo que cambió desde la redacción original: (1) el alta admin de empleado ya NO hace N escrituras sueltas desde la server action sino **una** llamada atómica al RPC `hr.create_employee_with_invite` (SECURITY DEFINER, all-or-nothing — CODE-ADMIN-TX, migraciones 068/070/071); las escrituras transaccionales del wizard van por `hr.complete_onboarding_writes`. (2) Los nombres/ramas concretas de `onboardEmployee()` descritos abajo son ilustrativos del diseño, no un mapeo línea-a-línea del código actual. Para el comportamiento exacto, la fuente de verdad es el código + la tabla de RPCs en `docs/reference/schemas-permisos.md`.
