...
**hr.generate_employee_code** (funcion, SIGNUP-formula):
Generador del `employee_code` PO-06 (ADR-0036): **3 letras del apellido paterno** (sin acentos, UPPER; el apellido es PARAMETRO, nunca derivado de `full_name`) + **3 ultimos digitos de la cedula** (CUCALON + 8-930-2166 -> CUC166). Colision contra `upper(employee_code)` -> **bump determinista del ultimo char alpha-first** (CUC166 -> CUC16A, luego ...B, y al final digitos), con guard `v_cand <> v_base`. `SECURITY DEFINER search_path=''`, **service_role-only**; NO escribe `hr.people` (el caller persiste). El indice `hr.people_code_ci_unique` (UNIQUE sobre `upper(employee_code)`) es la garantia real de unicidad; la funcion es best-effort ante carreras.
_Avoid_: re-derivar la formula para alguien ya en Spectrum (reproduce/colisiona su codigo existente); derivar el apellido de `full_name`; exponer la funcion a anon/authenticated.

**Reconciliacion employee_code <-> Spectrum** (ADR-0036):
El link de identidad **DURABLE** es el crosswalk `hr.person_sources` (clave externa por source_system), **NO** la columna `employee_code`. `hr.sync_spectrum_people` resuelve **crosswalk-first** y luego `employee_code` case-insensitive. Las personas **spectrum-sourced conservan su codigo Spectrum**; `hr.generate_employee_code` corre **SOLO para net-new sin crosswalk spectrum**. La fortuita coincidencia "Spectrum code == formula PO-06 al ~90%" es empirica, no contractual.
_Avoid_: tratar `employee_code` como la identidad cross-app (es el crosswalk); generar un codigo nuevo para una persona que ya tiene fila en Spectrum.
