...
- **2026-06-05 — Phase 4: Group 4 engines + VACACIONES — DESIGN-ONLY** — DONE (no DB/code/apply, by scope). Commit pending.
  - **Produced:** `docs/superpowers/specs/2026-06-05-group4-engines-vacaciones-design.md` (the 6 engines + interfaces + data flow; VACACIONES `form_schema` + `approval_chain_template` + computed fields; 8-status/4-mode state machine; the migration/code TASKS for the attended build) + **ADR-0037** (Proposed, ratification PENDING Jaime).
  - **Key proposed decision (ADR-0037):** `requests.approvals` needs a `kind` column (`submit|approval|processing`) — ADR-0027 mandated `kind` per step but the runtime table had no home for it (introspection confirmed absent); the ApprovalEngine gates only on `kind='approval'`. AND reinstate `sequential` mode (ADR-0020 eliminated it, conflating gate-ORDER with RRHH-VISIBILITY; VACACIONES PO-05 §5.9 proves order is load-bearing — GG signs LAST after Planilla verifies acumulados; `visibility:"universal"` keeps RRHH day-0 regardless of mode). Amends ADR-0020 as a proposal.
  - **Also closes/routes:** BL-4 (annual reset — `{year}` already disambiguates, cosmetic), BL-5 (`Devuelta_Info` orphan — don't wire), DB-1 (audit-trigger decided inside the engines migration). Reuse `hr.post_leave_ledger_entry` (exists).
  - **NOT built (attended):** the `approvals.kind` migration + the 6 engines + VACACIONES seed + ChainResolver. This is a reviewed architecture head-start, not an implementation. Needs Jaime's ratification of ADR-0037 before the attended build.
- **2026-06-05 — Phase 5: Group 4 engines BUILD — WIP, NOT applied, HANDOFF to a fresh session.**
  - **Also done + committed this stretch:** `092_login_resolver_and_login_rate_limit` APPLIED + Council B auth-flow committed (`1b2cde8`); backup `backup.auth_users_20260605` + `backup.public_manifest_20260605`. `093` invite-guard DEFERRED (reimplement as additive trigger on `hr.invite_codes`).
  - **Engines build files are IN THE TREE, uncommitted (WIP):** `src/lib/engines/**` (approval-engine / chain-resolver / form-engine / stamp-engine + tests), the VACACIONES actions, `supabase/migrations/20260605081100_093_approvals_kind_vacaciones_engines.sql` (ALTER `requests.approvals` ADD `kind` + VACACIONES `requests.types` re-seed: sequential chain submit→RRHH(processing)→supervisor(approval)→Planilla(processing)→GG/president(approval LAST) + leave-ledger RPCs), `supabase/tests/database/093_engines_vacaciones.test.sql`. **The migration is NOT applied (live HEAD = `092` / `20260605110113`).**
  - **Why NOT applied (two adversarial reviews — the build council died before its own review):**
    - **[BLOCKING B1]** `instantiate_chain` inserts `approver_role='requester'` for the submit step, but the live `requests.approvals.approver_role` CHECK only allows `supervisor|hr_admin|president|specific_person` → every VACACIONES submit throws `23514` + rolls back. The council's self-reported green pgTAP was **theater** (TS twin tests pass; the DB path is dead).
    - **[R26 / S1]** sequential ordering not enforced across processing steps → GG/president could approve **before** Planilla verifies the balance (violates PO-05 §5.9, the whole reason for `sequential`).
    - **[S2]** `process_step` doesn't advance the `current_step`/`current_assignee_id` pointer (comment lies). **[S3]** submit notifies the supervisor prematurely (RRHH-recibe is first). **[N3]** `apply_leave_effect` release/reject updates `leave_balances` but posts no `leave_ledger` reversal row (audit gap vs spec §6.3).
  - **Remediation STARTED but its agent hit a socket error mid-run** → the migration's edits (S1 ordering guard + N3 reversal markers present) are **partial + UNVERIFIED**; `requester` may still be inserted. **Do NOT apply without a full re-review.**
  - **Fresh-session steps:** (1) re-review the migration end-to-end (the errored remediation may be inconsistent) — confirm B1 (no `requester` approvals row / submit row skipped), S1 (out-of-order RAISE), S2/S3/N3. (2) `migration-reviewer` PASS. (3) apply `093` (renumber file ts — `20260605081100` sorts before applied `092`). (4) `supabase gen types --linked` (engines TS references `requests.approvals.kind`). (5) `npm run verify`. (6) commit. Design authority: ADR-0037 + `docs/superpowers/specs/2026-06-05-group4-engines-vacaciones-design.md`. Infra note: the engines council died twice + MCP dropped + the remediation agent socket-closed this session — a clean session is the right place to land this core/R26-sensitive slice.
