...
- **W0.5 — Framework hardening** (`framework-hardening-design.md`): **HARDEN-NOW:** (a) escalar anti-voseo eslint a `error` (sigue en `warn` pese a que memory/CLAUDE.md dicen H9-error — confirmar count=0); (b) PostToolUse BOM-check (exit-2) en `.json/.ps1/.md` (R23 hoy NO está enforced por hook); (c) router matcher con word-boundary regex (hoy `.Contains()` → falsos positivos) + marcar routing como *advice, no gate*; (d) arreglar contradicción en `06-FRAMEWORK` (dice Stop hook "No implementado" pero `stop.ps1` existe y está registrado); (e) gate de doc-maintenance (que falle si el estado driftea fuera de STATUS.md). **Los 2 grandes habilitadores de overnight (PROVISION→probar en Group 3):** Stop-hook que **re-entra al loop hasta que `verify` pase** (hoy verify es gate manual), y **reviewer adversarial cross-model (Codex sobre el diff)** antes de merge. **DEFER:** enforcement de R2-R21/R24-R27 al ApprovalEngine (Group 4) + `sop-chain-auditor` como step requerido del review.
