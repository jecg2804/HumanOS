...
- P2.24 (COMMENT backfill, subset): `[bd] 051_backfill_comments_leave_requests` — column comments para las tablas live/near-term: `hr.leave_*` (foundation del 047) + `requests.*` + `notifications.outbox`. Los comments de `learning.*`/`performance.*`/`workflows.*` se difieren a su grupo de feature (semántica final).
