...
PROVISION (W1 batch 4: schema `mdm` + DOMAIN `mdm.source_system` + pgvector + `audit.log.source_system` + SEC-CONSENT) y TYPES-STALE (regenerar `database.types.ts`) pendientes al cierre de W1.
